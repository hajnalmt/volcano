...
// meters (checked via api.Intersection against queue.Deserved), so the queue
// usage is unaffected by scheduling this task.
...
func (pmpt *Action) pipelineWithIdleResources(
	...
	queueDeserved := api.NewResource(currentQueue.Queue.Spec.Deserved)
	if len(api.Intersection(preemptor.InitResreq, queueDeserved)) > 0 {
